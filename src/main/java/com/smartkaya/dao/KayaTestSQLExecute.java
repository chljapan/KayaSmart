...
public final class KayaTestSQLExecute {
	...
	public List<Map<String,String>> selectByBusinessKeys(Paramater paramater) {
		...
		if (Constant.G_PRODUCT.equals(AccessKayaModel.getParentKayaModel(kayaModelId).getMetaModelType())) {
			...
			selectSQL.append(" WHERE parentid = '" + kayaModelId
					+ "' AND businessid = '" + KayaModelUtils.getBusinessKey(AccessKayaModel.getParentKayaModel(kayaModelId),paramater.getMapping().getKeys()) 
	...
	public int delete(Paramater paramater) {
		...
		StringBuilder deleteSQL = new StringBuilder("DELETE "
				+ "FROM " + tableName);
		deleteSQL.append(" WHERE orientationkey like '" + KayaModelUtils.getOrientationKey(AccessKayaModel.getKayaModelId(kayaModelId),paramater.getMapping().getKeys()) + "%';");
	...
	public int delete(Paramaters paramaters) {
		...
		for(Mapping mapping:paramaters.getMappings()) {
			...
			if (Constant.G_PRODUCT.equals(AccessKayaModel.getParentKayaModel(kayaModelId).getMetaModelType())){
				// orientationkey
				orientationKey =  KayaModelUtils.getBusinessKey(AccessKayaModel.getKayaModelId(kayaModelId),mapping.getPropertys());
				// 否则判定为子表(更新子表的时候需要BusinessID作为主键更新)
			} else {
				// orientationkey
				orientationKey =   KayaModelUtils.editOrientationKey(AccessKayaModel.getKayaModelId(kayaModelId),KayaModelUtils.getBusinessKey(AccessKayaModel.getParentKayaModel(kayaModelId),mapping.getPropertys()),mapping.getPropertys());
			}
			String tableName = AccessKayaModel.getKayaModelId(kayaModelId).getTableId().replace('-','_');
			StringBuilder deleteSQL = new StringBuilder("DELETE "
					+ "FROM " + tableName);
	...
	public int delete(List<Paramaters> paramatersList) {
		...
		for (Paramaters paramaters:paramatersList) {
			...
			for(Mapping mapping:paramaters.getMappings()) {
				...
				StringBuilder deleteSQL = new StringBuilder("DELETE "
						+ "FROM " + tableName);
				deleteSQL.append(" WHERE orientationkey like '" + KayaModelUtils.getOrientationKey(AccessKayaModel.getKayaModelId(kayaModelId),mapping.getKeys()) + "%';");
				System.out.println(deleteSQL);
				sqlStringList.add(deleteSQL.toString());
			}
		}
	...
	private String getWorkflowStartSqlString(String kayaModelId, String workflowId, Mapping mapping) {
	...
	// 主表子表主键处理
	private void getInsertSqlString(Mapping maping,String kayaModelId, StringBuilder insertSQL,String orientationKey,List<String> sqlStringList,boolean flg){
		// 取得Role子元素信息
		List<KayaMetaModel> kayaModelList = AccessKayaModel.getKayaModelByParentIdNotRole(kayaModelId);
		// 取得ParentRole信息
	...
	private void getUpdateSqlString(Mapping maping,String kayaModelId,List<String> sqlStringList,List<String> insertFieldList){
